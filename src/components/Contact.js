...
export const Contact = () =>{
    ...
    const handleSubmit = async(e) => {
        ...
        let response = await fetch("httlp://localhost:3001/contact" , {
            method: "POST",
            headers: {
        ...
        if(result.code === 200){
            setStatus({succces: true, message: "Message sent successfully"});
        } else {
            setStatus({succces: false, message: "Something went wrong"})
        }
    };
                        ...
                        <form onSubmit={handleSubmit}>
                            <Row>
                                ...
                                </Col>
                                {
                                    status.message && 
                                    <Col>
                                    <p className={status.succces == false? "danger" : "success"}>{status.message}</p>
                                    </Col>
                                }
                            </Row>
                        </form>
